...
let fetchVenue = async () => {
    try {
        let res = await fetch("http://localhost:3000/venues");
        let venues = await res.json();
        let cVenues = venues.filter((el) => {
            return el.location.includes(data[0].name) || data[0].name.includes(el.location);
        })
        console.log(cVenues);
        displayUi(cVenues);
...
function displayUi(arr){
    ...
    document.querySelector("#selected-location").textContent = data[0].name;

    if (arr.length == 0) {
        let d = document.createElement("div");
        d.setAttribute("id", "empty")
        let empty = document.createElement("img");
        empty.src = "https://playo-website.gumlet.io/playo-website-v2/Layer+2.svg"
        d.append(empty)
...
//Codr for filter function 
    ...
    async function filterFunction(value, div, text){
        console.log(value);
        try {
            let res = await fetch("http://localhost:3000/venues")
            let d = await res.json();
            let filter_array = d.filter((el) => {
